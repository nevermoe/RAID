...
bitmap_store(mddev_t *mddev, const char *buf, size_t len)
{
	...
	mddev->bitmap->flags |= BITMAP_SET_BITS;
...
static ssize_t
bits_flag_show(mddev_t *mddev, char *page)
{
	if (!mddev->bitmap) {
		return sprintf(page, "%s\n", "NULL");
	} else {
		return sprintf(page, "%d\n", 
			(mddev->bitmap->flags & BITMAP_SET_BITS) ? 1 : 0);
	}
}

static ssize_t
bits_flag_store(mddev_t *mddev, const char *buf, size_t len)
{
	int set_bit = simple_strtoul(buf, NULL, 0);

	if (!mddev->bitmap)
		goto out;
	if (set_bit < 0 || set_bit > 1)
		goto out;
...
static struct md_sysfs_entry md_bitmap_bits =
__ATTR(bitmap_bits_flag, S_IRUGO|S_IWUSR, bits_flag_show, bits_flag_store);
